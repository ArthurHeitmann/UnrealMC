...
void AChunk::SetMeshData(TMap<TEnumAsByte<EAllBlocks>, TArray<FVector>>& pTmpVertecies, TMap<TEnumAsByte<EAllBlocks>, TArray<FVector2D>>& pTmpUVs, TMap<TEnumAsByte<EAllBlocks>, TArray<int32>>& pTmpTriangles, TMap<TEnumAsByte<EAllBlocks>, TArray<FVector>>& pTmpNormals, TMap<TEnumAsByte<EAllBlocks>, Block*>& pTmpMaterials)
{
	TmpVertecies = pTmpVertecies;
	TmpUVs = pTmpUVs;
	TmpTriangles = pTmpTriangles;
	TmpNormals = pTmpNormals;
	TmpMaterials = pTmpMaterials;
	bHasTmpMeshChanged = true;
}
...
void AChunk::QuickUpdateMesh()
{
	if (!bHasTmpMeshChanged)
		return;

	for (auto i = TmpVertecies.CreateConstIterator(); i; ++i)
	{
		TEnumAsByte<EAllBlocks> key = i.Key();
		ChunkMesh->CreateMeshSection(key, i.Value(), TmpTriangles[key], TmpNormals[key], TmpUVs[key], TArray<FColor>(), TArray<FProcMeshTangent>(), true);
		ChunkMesh->SetMaterial(key, TmpMaterials[key]->GetMaterial(this));
	}

	bHasTmpMeshChanged = false;
}
...
Block* AChunk::RegisterHitAt(const FHitResult& HitResult, Item* Item)
{
	...
	int xChange = -1;
	int yChange = -1;
 	if (HitResult.ImpactPoint.X < 0)
	{
		HitLocation.X += 15;	
		xChange = 1;
	}
	if (HitResult.ImpactPoint.Y < 0)
	{
		HitLocation.Y += 15;
		yChange = 1;
	}
